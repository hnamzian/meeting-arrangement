...
class Greeter {
    ...
    _hasInterferenceWithMeeting(newMeeting, refMeeting) {
        if (newMeeting[0] >= refMeeting[1] || newMeeting[1] <= refMeeting[0]) {
            return false;
        }
        return true;
    }
    _hasInterferenceWithRoom(meeting, room) {
    ...
    _hasFreeRoomForMeeting(meeting, rooms) {
        let freeRoomId = 0;
    ...
    _extendRoomMeetingTime(roomId, rooms, meeting) {
        if (roomId !== null) {
